		...
		for (int x = 0; x < _map->getWidth(); x++) {
			for (int y = 0; y < _map->getHeight(); y++) {
				if (z == 0) {
					...
					if (_map->tileMap.at(z).at(x)[y] == ' ') _map->sprite[z].setTextureRect(IntRect(650, 650, 32, 32)); //рисуем пустоту вторым и последующими слоями над травой
				}

				if (_map->tileMap.at(z).at(x)[y] == 'r') _map->sprite[z].setTextureRect(IntRect(96, 672, 32, 32));//рисуем реку
				if (_map->tileMap.at(z).at(x)[y] == '0') _map->sprite[z].setTextureRect(IntRect(96, 32, 32, 32));//рисуем стену возле края карты
				if (_map->tileMap.at(z).at(x)[y] == 'b') _map->sprite[z].setTextureRect(IntRect(320, 32, 32, 32));//рисуем мост
